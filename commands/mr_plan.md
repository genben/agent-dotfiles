...
The Living Sections in `plan.md` must include the maintainer instructions from the template (`~/.agents/templates/plan_template.md`). These instructions guide future contributors on how to maintain the Decision Log, Progress, Surprises & Discoveries, and Outcomes sections. Do not remove these instructions from the final plan.

## Templates (required)
Use these templates as the starting structures:
- `~/.agents/templates/plan_template.md` -> for `plan.md`
- `~/.agents/templates/plan_phase_template.md` -> for each `plan_phase_<N>.md`
