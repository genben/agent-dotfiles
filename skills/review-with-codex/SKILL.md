...
Non-interactive, in the background; `-o` captures the final message as the report:

```bash
codex exec -o {report-path} "$(cat {brief-path})"
```
...
- Interactive codex has no `--name`. Resolve the thread id from the brief's marker line once the session starts; match the marker, never "the newest thread". Record the id.
